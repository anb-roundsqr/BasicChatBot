...
def get_choices_dict(choices):
    res = {}
    for db_val, ui_val in choices:
        res[db_val] = ui_val
    return res


def get_reverse_dict(dictn):
    res = {}
    if dictn:
        res = {v: k for k, v in dictn.items()}
    return res


def get_choices_dicts(choices):
    mapping = get_choices_dict(choices)
    reverse_mapping = get_reverse_dict(mapping)
